          ...
          if (isset(\$_FILES['ipa_file']) && isset(\$_FILES['mobileprovision_file']) && isset(\$_FILES['p12_file']) && isset(\$_POST['password'])) {
              ...
              if (\$ipaFile['error'] === UPLOAD_ERR_OK && \$mobileprovisionFile['error'] === UPLOAD_ERR_OK && \$p12File['error'] === UPLOAD_ERR_OK) {
                  ...
                  # Move the uploaded files to the target directory
                    if (rename(\$tempIpaFile, \$targetIpaFile) &&
                        rename(\$tempMobileprovisionFile, \$targetMobileprovisionFile) &&
                        rename(\$tempP12File, \$targetP12File)) {
...
main() {
    ...
    sleep 1
    compile_zsign
    echo -e "\e[32mSuccessfully installed and compiled SelfSign,\e[0m"
    cd ..
    chmod +xrw start.sh
}
main
